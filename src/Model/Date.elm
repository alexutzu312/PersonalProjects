...
monthsBetween dA dB =
    ...
    case (dA1.month, dB1.month) of
        ...
        (Just x, Just y) -> let
                                ...
                                monthB = monthToInt y
                            in
                                case Basics.compare yearA yearB of
                                    LT -> Just ((yearB - yearA) * 12 + monthB - monthA)
                                    GT -> Just ((yearA - yearB) * 12 + monthA - monthB)
                                    EQ -> Just (abs (monthB - monthA))
    ...
    let 
        firstOrder = 
            case (d1.month, d2.month) of
                ...
                (Nothing, Just x) -> GT
                (Just x, Nothing) -> LT
        secondOrder = Basics.compare d1.year d2.year
    in
        chainCompare (firstOrder) (secondOrder)
...
view (Date d) =
     let  
            monthExists : Date -> String
            monthExists (Date date) =
                    case (date.month) of
                    (Nothing) -> ""
                    (Just x)-> (monthToString  x)
     in
      div [] [
          text <| ( toString(d.year) ++ " " ++ (monthExists <| (Date d)))
      ]
